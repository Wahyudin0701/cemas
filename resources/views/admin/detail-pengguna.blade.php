            ...
            <div class="bg-white p-6 rounded-xl shadow border">
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-6">
                        ...
                        <p><span class="font-semibold">Tanggal Daftar:</span> {{ $user->created_at->format('d M Y') }}</p>
                    </div>
                ...
                <div class="mt-6 border-t pt-5">
                    ...
                    <div class="text-sm space-y-3">
                        <p><span class="font-semibold">Nama Toko:</span> {{ $user->penjual->toko->nama_toko }}</p>
                        <p><span class="font-semibold">Jam Buka:</span> {{ $user->penjual->toko->jam_buka ?? '-' }}</p>
                        <p><span class="font-semibold">Jam Tutup:</span> {{ $user->penjual->toko->jam_tutup ?? '-' }}</p>
                        <p><span class="font-semibold">Alamat:</span> {{ $user->penjual->toko->lokasi ?? '-' }}</p>
                        <p><span class="font-semibold">Deskripsi:</span> {{ $user->penjual->toko->deskripsi_toko ?? '-' }}</p>
                    </div>
...
<div
    ...
    <div
        ...
        <h2 class="text-xl font-bold text-gray-900 mb-3 text-center">
            <span x-text="actionType === 'approve' ? 'Konfirmasi Verifikasi Toko' : 'Konfirmasi Penolakan Toko'"></span>
        </h2>
        ...
        <div class="flex justify-center gap-4">
            ...
            <form action="{{ route('admin.update-status-toko', $penjual->toko->id) }}" method="POST">
                ...
                <button
                    class="px-5 py-2 text-sm rounded-lg transition font-medium shadow"
    ...
    <div
        ...
        <p class="text-sm text-gray-700 mb-6 text-center leading-relaxed">
            ...
            <span class="font-semibold text-gray-800">
                {{ $penjual->is_active ? 'menonaktifkan akun ini?' : 'mengaktifkan akun ini?' }}
            </span>
        </p>
        ...
        <div class="flex justify-center gap-4">
            ...
            <!-- KONFIRMASI -->
            <form action="{{ route('admin.toggle-status-user', $penjual->user->id) }}" method="POST">
                @csrf
                <button
                    class="px-5 py-2 text-sm rounded-lg {{ $penjual->is_active ? 'bg-yellow-500 hover:bg-yellow-600 text-white' : 'bg-green-600 hover:bg-green-700 text-white' }} transition font-medium shadow">
                    {{ $penjual->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                </button>
            </form>
        </div>
