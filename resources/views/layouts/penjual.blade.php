...
<head>
    ...
    <title>@yield('title', 'Penjual Dashboard')</title>
    <!-- Favicon -->
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-slate-200 transition-transform duration-300 transform lg:static lg:translate-x-0 flex flex-col shadow-sm">
            ...
            <div class="flex items-center justify-between h-16 px-6 border-b border-slate-100 shrink-0">
                <a href="/" class="flex items-center gap-3 group py-2">
                    ...
                    <span class="text-xl font-extrabold text-primary-600 tracking-tight group-hover:text-primary-700 transition-colors">CeMas Penjual</span>
                </a>
            ...
            <div class="flex-1 px-4 py-6 overflow-y-auto sidebar-scroll space-y-1">
                ...
                <a href="{{ route('penjual.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold transition-colors {{ request()->routeIs('penjual.dashboard') ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-primary-600' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('penjual.dashboard') ? 'text-primary-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                    Dashboard
                </a>
                
                <p class="px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Manajemen Toko</p>
                <a href="{{ route('penjual.kelola-produk') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold transition-colors {{ request()->routeIs('penjual.kelola-produk') || request()->routeIs('penjual.tambah-produk') || request()->routeIs('penjual.edit-produk') || request()->routeIs('penjual.detail-produk') ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-primary-600' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('penjual.kelola-produk') || request()->routeIs('penjual.tambah-produk') || request()->routeIs('penjual.edit-produk') || request()->routeIs('penjual.detail-produk') ? 'text-primary-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                    Kelola Produk
                </a>

                <a href="{{ route('penjual.pesanan') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold transition-colors {{ request()->routeIs('penjual.pesanan*') ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-primary-600' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('penjual.pesanan*') ? 'text-primary-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                    Pesanan Masuk
                </a>
                
                <a href="{{ route('penjual.toko') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold transition-colors {{ request()->routeIs('penjual.toko*') ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-primary-600' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('penjual.toko*') ? 'text-primary-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                    Profil Toko
                </a>
                    ...
                    <div class="relative flex" x-data="{ open: false }" @click.away="open = false">
                        <button type="button" @click="open = !open" class="flex items-center gap-3 text-sm text-slate-600 hover:text-slate-900 transition-colors cursor-pointer focus:outline-none" aria-haspopup="true" :aria-expanded="open.toString()">
                            <div class="text-right hidden md:block">
                                ...
                                <p class="text-[10px] font-bold text-primary-600 uppercase tracking-wider">Penjual</p>
                            </div>
