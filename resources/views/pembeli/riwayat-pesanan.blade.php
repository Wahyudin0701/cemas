    ...
    <p class="text-gray-600 mt-2 mb-10">
        ...
        @if($orders->isEmpty())
            <div class="text-center py-20 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
                <svg class="w-20 h-20 text-gray-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
                <h3 class="text-xl font-medium text-gray-400">Belum ada pesanan</h3>
                <a href="{{ route('pembeli.dashboard') }}" class="mt-4 inline-block px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Belanja Sekarang</a>
            </div>
        @else
            <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition">
                        ...
                        <div class="p-6">
                                    ...
                                    <div class="flex items-start gap-4">
                                        <div class="w-16 h-16 bg-gray-100 rounded-md overflow-hidden flex-shrink-0">
                                             @if($detail->produk->foto_produk)
                                                <img src="{{ asset($detail->produk->foto_produk) }}" class="w-full h-full object-cover" alt="{{ $detail->produk->nama_produk }}">
                                            @else
                                                <div class="w-full h-full flex items-center justify-center text-gray-400 text-xs">IMG</div>
                                            @endif
                                        </div>
                                        <div class="flex-1">
                                            <h4 class="text-sm font-medium text-gray-900">{{ $detail->produk->nama_produk }}</h4>
                                            <p class="text-xs text-gray-500">{{ $detail->kuantitas }} x Rp {{ number_format($detail->harga_saat_pesan, 0, ',', '.') }}</p>
                                        </div>
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <div class="text-lg font-bold text-blue-600">Rp {{ number_format($order->total_harga_final, 0, ',', '.') }}</div>
                            </div>
                        </div>
                    </div>
                @endforeach
