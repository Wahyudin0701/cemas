            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="p-4 text-sm font-semibold text-gray-600">Catatan</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="p-4">
                                @if ($order->catatan_pembeli)
                                    <div class="text-xs text-gray-600 italic max-w-[150px] truncate"
                            ...
                            <td class="p-4">
                                <form action="{{ route('penjual.pesanan.update', $order->id) }}" method="POST">
                                    ...
                                    @if ($order->status_pesanan == 'Menunggu Konfirmasi')
                                        <button type="submit"
                                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-1 px-3 rounded text-xs transition duration-150">
                                            Konfirmasi Pesanan
                                        </button>
                                    @elseif($order->status_pesanan == 'Dikonfirmasi/Diproses')
                                        <button type="submit"
                                            class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-1 px-3 rounded text-xs transition duration-150">
                                            Tandai Siap
                                        </button>
                                    @elseif($order->status_pesanan == 'Siap Diambil/Diantar')
                                        <button type="submit"
                                            class="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-1 px-3 rounded text-xs transition duration-150">
                                            Selesai
                                        </button>
                                    @elseif($order->status_pesanan == 'Selesai')
                                        <span
                                            class="text-green-600 text-xs font-semibold flex items-center justify-center gap-1">
                                ...
                                @if (in_array($order->status_pesanan, ['Menunggu Konfirmasi', 'Dikonfirmasi/Diproses']))
                                    <form action="{{ route('penjual.pesanan.cancel', $order->id) }}" method="POST"
                                        class="mt-2"
                                        onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pesanan ini? Aksi ini tidak dapat dibatalkan.');">
                                        @csrf
                                        <button type="submit"
                                            class="w-full text-red-500 hover:text-red-700 hover:bg-red-50 font-medium py-1 px-3 rounded text-xs transition duration-150 border border-red-200">
                                            Batalkan Pesanan
                                        </button>
                                    </form>
                                @endif
